	...
	public void draw(Graphics g) {
		...
		for(int i = 0 ; i < sections ; i++) {
			...
			if(RoundData.statsContainer.containsKey(roundNumber)) {
				...
				if(Math.abs(stats.energyBalance()) > maxEnergyDisplayed) {
					maxEnergyDisplayed = Math.abs(stats.energyBalance())+50;
				}
			}
		}
		...
		g.drawString("Energy Overview", x + 30, y + 45);
		...
		if(animation != null) {
			for(int i = 0 ; i < sections ; i++) {
				...
				if(RoundData.statsContainer.containsKey(roundNumber)) {
					...
					RoundStatsContainer stats = RoundData.statsContainer.get(roundNumber);
					int energyBalance = stats.energyBalance();
					if(energyBalance > maxEnergyDisplayed) { energyBalance = maxEnergyDisplayed; }
					int sectionHeight = (int) ((energyBalance/(double) maxEnergyDisplayed)*(tableHeight/2));
					...
					if(energyBalance >= 0) {
						...
						g.drawString(""+energyBalance, tableX+sectionWidth*i+sectionBorder, tableY-displayedHeight-5);
					}else {
						g.fillRect(tableX+sectionWidth*i+sectionBorder, tableY+1, sectionWidth-sectionBorder*2, (displayedHeight*-1)-1);
						g.drawString(""+energyBalance, tableX+sectionWidth*i+sectionBorder, tableY-displayedHeight+20);
					}
