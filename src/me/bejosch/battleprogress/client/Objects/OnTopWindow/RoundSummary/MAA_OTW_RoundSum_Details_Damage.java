...
public class MAA_OTW_RoundSum_Details_Damage extends MouseActionArea {

	public MAA_OTW_RoundSum_Details_Damage() {
		super(WindowData.FrameWidth/2+5, WindowData.FrameHeight/2-OnTopWindowData.roundSum_height/2+100-25+OnTopWindowData.roundSum_smallDistanceBetween*2+OnTopWindowData.roundSum_bigDistanceBetween*1
				, WindowData.FrameWidth/2+OnTopWindowData.roundSum_width/2-5, WindowData.FrameHeight/2-OnTopWindowData.roundSum_height/2+100+10+OnTopWindowData.roundSum_smallDistanceBetween*4+OnTopWindowData.roundSum_bigDistanceBetween*1
				, "OTW_RoundSum_Details_Damage", null, ShowBorderType.ShowAlways, Color.WHITE, Color.ORANGE);
		this.OTWMMA = true;
	}
	...
	public void draw(Graphics g) {
		...
		if(this.isHovered()) {
			...
			int dealtRows = rsc.getDamageDealt_building().size()+rsc.getDamageDealt_troup().size();
			int receivedRows = rsc.getDamageReceived_building().size()+rsc.getDamageReceived_troup().size();
			int rowHeight = (dealtRows+receivedRows)*OnTopWindowData.roundSum_details_smallHeight;
			...
			g.drawString("Damage details", x + 20, y);
			
			y += OnTopWindowData.roundSum_details_bigHeight;
			g.setColor(Color.LIGHT_GRAY);
			g.setFont(new Font("Arial", Font.CENTER_BASELINE, 22));
			g.drawString("Dealt:", x + 20, y);
			
			for(Building b : rsc.getDamageDealt_building().keySet()) {
				int amount = rsc.getDamageDealt_building().get(b);
			...
			for(Troup t : rsc.getDamageDealt_troup().keySet()) {
				int amount = rsc.getDamageDealt_troup().get(t);
				y += OnTopWindowData.roundSum_details_smallHeight;
				g.setColor(Color.WHITE);
				g.setFont(new Font("Arial", Font.CENTER_BASELINE, 18));
			...
			g.setColor(Color.LIGHT_GRAY);
			g.setFont(new Font("Arial", Font.CENTER_BASELINE, 22));
			g.drawString("Received:", x + 20, y);
			
			for(Building b : rsc.getDamageReceived_building().keySet()) {
				int amount = rsc.getDamageReceived_building().get(b);
				y += OnTopWindowData.roundSum_details_smallHeight;
				g.setColor(Color.WHITE);
				g.setFont(new Font("Arial", Font.CENTER_BASELINE, 18));
				g.drawString(b.name+":", x + 20, y);
			...
			for(Troup t : rsc.getDamageReceived_troup().keySet()) {
				int amount = rsc.getDamageReceived_troup().get(t);
				y += OnTopWindowData.roundSum_details_smallHeight;
				g.setColor(Color.WHITE);
				g.setFont(new Font("Arial", Font.CENTER_BASELINE, 18));
				g.drawString(t.name+":", x + 20, y);
