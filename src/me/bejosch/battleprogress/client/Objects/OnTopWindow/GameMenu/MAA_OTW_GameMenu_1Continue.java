...
public class MAA_OTW_GameMenu_1Continue extends MouseActionArea {

	private static int menuPosition = 0;
	
	public MAA_OTW_GameMenu_1Continue() {
		super(WindowData.FrameWidth/2 - OnTopWindowData.menu_buttonWidth/2, WindowData.FrameHeight/2 - OnTopWindowData.menu_height/2+OnTopWindowData.menu_distanceToTopBorder+(menuPosition*OnTopWindowData.menu_distanceBetweenButton)
				...
				, "OTW_GameMenu_Continue", null, ShowBorderType.ShowAlways, Color.WHITE, Color.ORANGE);
		this.OTWMMA = true;
	}
	...
	public void draw(Graphics g) {
		...
		if(this.isActiv() == true) {  
			if(checkArea(MouseHandler.mouseX, MouseHandler.mouseY) == true) {
				...
				g.drawString("Continue", this.xTL+74, this.yTL+31);
			}else {
				//NO HOVER
				g.setColor(standardColor);
				g.setFont(new Font("Arial", Font.PLAIN, 19));
				g.drawString("Continue", this.xTL+73, this.yTL+30);
			}
		}
