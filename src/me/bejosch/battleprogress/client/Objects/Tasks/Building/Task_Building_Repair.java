...
public class Task_Building_Repair extends Task_Building{

	public Task_Building_Repair(Building connectedBuilding) {
		super(connectedBuilding, Images.taskIcon_Building_Repair, "Repair", 11, false, true, 3, null);
	...
	public void action_Left_Release() {
		...
		if(GameData.dragAndDropTaskInputActive == true) {
			...
			if(targetField != null) {
				if(this.building.fieldIsIn_REPAIR_Range(targetField)) {
					//IN RANGE
				...
				new InfoMessage_Located("You need to drag and drop the task to a target building", ImportanceType.LOW, this.building.connectedField.X, this.building.connectedField.Y, true);
				this.removeFromActiveTask();
			}
		}
