...
public class Task_Building_Heal extends Task_Building{

	public Task_Building_Heal(Building connectedBuilding) {
		super(connectedBuilding, Images.taskIcon_Building_Heal, "Heal", 11, false, true, 3, null);
		String[] hoverText = {"This task heals an allied troup","All tasks are executed at the end of the round"};
	...
	public void action_Left_Release() {
		...
		if(GameData.dragAndDropTaskInputActive == true) {
			...
			if(targetField != null) {
				if(this.building.fieldIsIn_HEAL_Range(targetField)) {
					//IN RANGE
					...
					this.targetPath = new PathFinding_Algorithmus(new FieldCoordinates(this.building.connectedField), new FieldCoordinates(targetField), true).getPath(this.building.viewDistance, true, false);
				...
				new InfoMessage_Located("You need to drag and drop the task to a target troup", ImportanceType.LOW, this.building.connectedField.X, this.building.connectedField.Y, true);
				this.removeFromActiveTask();
			}
		}
